...
// (legacy, Newburyport-only) hand-drawn downtown box — superseded by commercialDowntown
...
for (const el of raw.elements) {
  ...
  if (t.building && t.building !== 'no') {
    ...
    // Untagged 'building=yes' blocks in the commercial district render as brick
    // commercial (downtown is almost all brick) unless they're something more specific.
    if (k === 'house' && areaM2 >= 90 && commercialDowntown(ring)) k = 'commercial';
...
// out as holes. This handles harbors / multiple landmasses (a peninsula town like Salem, with
// land across the harbor) and a simple monotonic coast (Newburyport) alike — no hardcoded land
// references, no latitude-sort assumption. Orientation is chosen so downtown (ORIGIN, px 0,0)
// stays on land, so it self-corrects for whatever coastline winding a town's data uses.
{
