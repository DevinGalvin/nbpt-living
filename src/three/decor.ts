...
// How full the parking lots are, by season: the waterfront lots are full on a July
// Saturday and empty in February, and a winter town with summer lots reads wrong.
const LOT_FILL = SEASON === 'summer' ? 30 : SEASON === 'fall' ? 22 : SEASON === 'spring' ? 18 : 11;
...
export function buildChunkDecor(world: WorldData, index: WorldIndex, key: string): ChunkDecor | null {
  ...
  if (shopsHere >= 2) {
    ...
    for (const ri of bucket.roads) {
      ...
      let flip = 1;
      walkLineD(r.p, core ? 64 : 120, (x, z, tx, tz) => {
        flip = bothSides ? -flip : oneSide;
        const h2 = hash32(Math.round(x * 3), Math.round(z * 3), 41);
        // a kerb about half taken downtown, a quarter elsewhere: the game is Clipper and
        // the town, and a street solid with parked cars was the loudest thing in it
        if (h2 % 100 > (core ? 48 : 26)) {
        ...
        for (const qi of bucket.roads) {
          ...
          if (rq.c === 'service') continue;
          if (distToPolylineSq(px2, pz2, rq.p) < (rq.w / 2 + 6) ** 2) return;
        }
  ...
  for (const pi of bucket.polys) {
      ...
      for (const r of aisles) {
        walkLineD(r.p, 21, (x, z, tx, tz) => {
          ...
          for (const sSide of [1, -1] as const) {
            ...
            if (h2 % 100 > LOT_FILL) continue;   // a lot is never full; the sea of cars was the town's loudest thing from above
    ...
    for (const w0 of rows) {
      ...
      for (let l0 = -obb.hl + 16; l0 <= obb.hl - 16 && cars < 110; l0 += 22) {
        ...
        if (h2 % 100 > LOT_FILL * 0.8) continue;
        if (!pointInPolyD(x, z, poly)) continue;
