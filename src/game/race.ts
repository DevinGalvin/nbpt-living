...
const ghostKey = (id: string, name: string) => `salem-race-${id}-ghost-${name}`;
...
// SHARED with Newburyport on purpose (both towns live on one origin): a kid types
// their name once and it rides with them between towns. Ghost pref shared too.
...
const boardKey = (id: string) => `salem-race-${id}-board`;
...
export class RaceRunner {
  ...
  /** dev hook: arm + start a course by id (nbpt.race('marblehead')) */
