...
| secret | how | notes |
|---|---|---|
| 🕳 **The Smugglers' Tunnel** | dig on the iron grate in the back lot off State Street; a brick corridor with a dogleg; up at Federal Street — OR take the branch off the first corner and come up in **Market Square**. Three ends (the three real tunnels), any of them an entrance. Grates sit on the nearest diggable, dry, unblocked spot to their seed (`findSpot`). | `SecretTunnel('brick', paths[], width, ends[])`: several polylines, explicit ends. **Every end is capped** (the corridor visibly stops), with iron rungs up the cap, a daylight shaft onto a pale patch, and 🪜 CLIMB OUT on the button within 44 px (auto-exit within 14). Devin 9/13: "theres no way to get out of the tunnel" — there was, an invisible 14 px spot. Game swaps scenes like the story tunnel (`enterSecretScene`/`exitSecretScene`); `Game.tunnel` is `TunnelScene \| SecretTunnel`, the story's lives in `storyTunnel`. |
