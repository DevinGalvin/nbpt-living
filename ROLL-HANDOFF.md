...
| 🏙 **The Weathervane** | a fire escape on the alley wall of the 4-storey building at Inn Street Mall (State Street block): 🪜 CLIMB on the contextual button; over the seam and DOWN onto the long 2-storey block that runs 630 px along State Street; touch the rooster vane at its far end (−268, 812) and it spins. A second ladder on the parking garage is a lookout only. | **Free-running** (Devin: "climbing only makes sense if he can naturally jump between buildings"): on the roofs `roofFree` allows any roof at this height, a step up ≤ 12 px, ANY drop (a thump + shake on landing), a leap over a gap when a roof is within 80 px ahead (`leap` → the fence hop, taller), and an edge with nothing beyond is walkable — `update()` then drops him to the street (`fallReq` → `Game.fallFromRoof`, OOF! + dust + shake). ⚠️ The fall is decided in `update()` from where he IS, never inside `roofFree` — that predicate is called on glance-off probe points and a probe past the edge dropped him. ⚠️ Ladders are placed from the building polygon (`LADDERS[].inside/toward` → nearest wall face, flush, brackets, hoop); the first cut stood one on a corner in mid-air. ⚠️ Roof chains were measured polygon-to-polygon: bounding boxes said Horton's Yard was 30 px from the garage; it is 200. |
