...
| 💨 **Zoomies** | `Game.startZoomies`, `updateCamera` | 2.4 s, 110 px/s at 12 rad/s = a ~9 px tail-chase. The anchor (`zoomCX/CZ/CY`) is the circle's CENTRE, and `updateCamera` frames the anchor, not the dog, for the whole spin — following him round even a 9 px circle made the world wobble (Devin: "the camera goes crazy"). The spin heading bypasses the `aimX/aimZ` smoother (it halved and jittered the circle). |
...
| secret | how | notes |
|---|---|---|
| 🕳 **The Smugglers' Tunnel** | **The story tunnel is an entrance too.** Devin 9/14: "when i go down the tunnel its just the dead end from the original story mode" — he meant Chapter 1's tunnel under the firehouse, the one a player knows. Once its collapse has been LOOKed at (step ≥ 5), a dog-sized gap at the foot of the rubble offers 🐕 SQUEEZE THROUGH (`TunnelScene.onSqueeze` → `Game.squeezeToNetwork` → `swapUnderground`, scene to scene, no daylight). The network's fourth end is that collapse from the other side (`NET_ENDS[3].squeeze`: a rubble pile, a dark gap, cold light) and squeezes back into the cache room. Otherwise: dig on the grate in the back lot off State Street; the brick corridor with a dogleg; up at Federal Street; or the branch off the first corner up into **Market Square**. Grates sit on the nearest diggable spot to their seed (`findSpot`). | `SecretTunnel('brick', paths[], width, ends[])`: several polylines, explicit ends (`squeeze?` marks a collapse end). **Every other end is capped** with iron rungs, a daylight shaft onto a pale patch, and 🪜 CLIMB OUT on the button within 44 px (auto within 14). Game swaps scenes like the story tunnel (`enterSecretScene`/`exitSecretScene`); `Game.tunnel` is `TunnelScene \| SecretTunnel`, the story's lives in `storyTunnel`; `Secrets.network()` builds the scene on first use. |
...
| 🏙 **The Weathervane** | a fire escape on the alley wall of the 4-storey building at Inn Street Mall: 🪜 CLIMB; over the seam and DOWN onto the long 2-storey block along State Street; touch the rooster vane at its far end (−268, 812). A second ladder on the parking garage is a lookout only (nothing within a leap of it). | **Real jumps** (Devin 9/13: "the roof jumps are far from natural" — they were the fence hop with the walk speed on). `roofFree` returns FALSE for anything off this roof and files a `jumpReq` (a lower roof → a drop-jump onto it; a gap with a roof within 80 px → the leap; nothing → a FALL to the street). Game takes the request only when the stick is pushed that way (the predicate is also called on the wall-glance and snout probes), then `startLeap`: a parabola sized to distance and drop, the stick off, `Dog.setAirborne` (front reaching, hind stretched, ears back), `Dog.land()` squash, a thump + dust on a drop, OOF! + shake on a fall. Ladders are placed on the nearest wall face of their building's own polygon (`LADDERS[].inside/toward`), flush, bracketed, hooped. Roof chains were measured polygon-to-polygon: the garage's neighbours are 200 px away, not 30. |
...
All five screenshot-verified (corridor, room card, ladder on the wall, the drop onto the long roof, vane card,
...
- **WAKE THE SIGN at Fowle's** (eggs.ts SPOTS 'fowles', the neon mesh, eggs2's shop cat, map.mjs curatedPoisHand): all four still carried Fowle's OLD coordinate (−109, 545) after the shop crossed to the east side of State Street (−33, 738). Devin 9/14. The neon is now mounted on the nearest wall face to the spot, turned to the street, in the window band; the cat sits a few windows down so the two hotspots do not fight.
- **The giant ice-cream cone in Pleasant Street** (decor.ts `ice_cream` POI → `iceCreamStand`): Harbor Creamery is a downtown storefront with no forecourt, and `forecourtSpot`'s fallback put the cone and both picnic tables on the kerb line among the parked cars (Devin 9/14: "what is this in the middle of the street" / "looks like an ice cream to me"). The stand now builds only where it lands on `plain`/`green`/`parking` at all three footprints and outside the brick core. A roadside stand with a lot still gets its cone.
- **The grave mist at Old Hill** (`GraveMist`/`mistTexture` in life.ts): eleven camera-facing sprites 150–320 px across at 0.72 opacity with a 0.9-alpha centre read as white saucers on the grass from the chase cam (Devin 9/14: "is this supposed to be fog?"). Now 44 puffs of 45–110 px, a third as tall, 3–7 px off the ground, a soft 0.42-centre gradient, 0.5 max opacity, each breathing on its own clock. Screenshot-verified at dusk and at night: a low haze, not objects.
